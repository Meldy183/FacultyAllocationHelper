package staff

import (
	"context"

	"gitlab.pg.innopolis.university/f.markin/fah/profileService/internal/domain/transaction"
)

type Repository interface {
	GetAllStaffByInstanceID(ctx context.Context, instanceID int64) ([]*Staff, error)
	GetStaffByInstanceAndVersionID(ctx context.Context, instanceID int64, versionID int64) (*Staff, error)
	AddStaff(ctx context.Context, tx transaction.Transaction, staff *Staff) error
	UpdateStaff(ctx context.Context, tx transaction.Transaction, staff *Staff) error
	DeleteStaff(ctx context.Context, tx transaction.Transaction, staffID int64) error
}
